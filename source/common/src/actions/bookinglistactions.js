...
export const updateBooking = (booking) => (dispatch) => async (firebase) => {
  ...
  if (booking.status == 'REACHED') {
    ...
    if(appcat == 'taxi'){
      ...
      const {totalCost, grandTotal, convenience_fees} = FareCalculator(distance, totalTimeTaken, rates, null, settings.decimal);
      booking.drop = { 
        add: address,
        lat: driverLocation.lat,
        lng: driverLocation.lng
      ...
      booking.driver_share = grandTotal - convenience_fees;
