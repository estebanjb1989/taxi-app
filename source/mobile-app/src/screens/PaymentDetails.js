  ...
  const useWallet = () => {
    ...
    if (res) {
      if (parseFloat(walletBalance) >= parseFloat(payDetails.payableAmount)) {
        ...
        data.payableAmount = (parseFloat(data.payableAmount) - parseFloat(walletBalance)).toFixed(settings.decimal);
        setPayDetails(data);
      }
    } else {
      let data = { ...payDetails };
          ...
          {userdata && userdata.usertype == 'rider'  && (booking.status == 'PAYMENT_PENDING' || (appcat == 'taxi' && booking.status == "PENDING"))? 
            <View style={{ flex: 1 }}>
              <CheckBox
                ...
                title={settings.swipe_symbol===false? t('use_wallet_balance') + settings.symbol + (parseFloat(walletBalance) - parseFloat(payDetails.usedWalletMoney)).toFixed(settings.decimal) + ')'
                  : t('use_wallet_balance') + (parseFloat(walletBalance) - parseFloat(payDetails.usedWalletMoney)).toFixed(settings.decimal) + settings.symbol + ')'}
