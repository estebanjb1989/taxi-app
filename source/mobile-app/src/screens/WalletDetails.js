...
export default function WalletDetails(props) {
  ...
  const rCom = auth.info && auth.info.profile && (auth.info.profile.usertype =='driver' || (auth.info.profile.usertype =='rider' && settings && settings.RiderWithDraw))?<TouchableOpacity onPress={doWithdraw}><Text style={{color:colors.WHITE}}>{t('withdraw')}</Text></TouchableOpacity>:null;
